...
class QuasiWolfeTest:
    def __init__(self, alphamax=10, eta_a=1e-4, eta_w=0.9, rho=2.0, max_iterations=1000, constraints=(-1, 1),
    ...
    def search(self, f, g, x, starting_gradient):
        ...
        while a_curr < self.alpha_max:
            ...
            # print(f" current try x - {x - a_curr * starting_gradient}")
            ...
            if (phi_a_curr > phi(0) + self.eta_a * a_curr * dphi_plus_0) or \
                ...
                success_2,a_star = self.stage_two(a_old, a_curr, phi, dphi_minus, dphi_plus,
                ...
                print(f" Stage two reached")
                return success_2, a_star, phi(a_star), np.array(stage_one_tries), np.array(stage_two_currents), np.array(
            ...
            if abs(dphi_minus_curr) <= self.eta_w * abs(dphi_minus_0) or \
                ...
                print(f" never reached stage two - step satisfies Wolfe conditions")
                return True, a_curr, phi_a_curr, np.array(stage_one_tries), np.array(stage_two_currents), np.array(
                    stage_two_brackets), stage_one_tries_alphas, stage_two_currents_alphas, stage_two_brackets_alphas
            ...
            if dphi_minus_curr >= 0:  # FIXME untested!
                success_2, a_star = self.stage_two(a_curr, a_old, phi, dphi_minus, dphi_plus,
                                        x, starting_gradient, phi_0, dphi_plus_0,
                                        stage_two_currents, stage_two_brackets,
                                        stage_two_currents_alphas, stage_two_brackets_alphas)
                print(f" Stage two reached")
            ...
            i += 1
        print(f" stage never reached returning alpha_max ")

        return False, self.alpha_max, phi(self.alpha_max), np.array(stage_one_tries), np.array(stage_two_currents), np.array(
            stage_two_brackets), stage_one_tries_alphas, stage_two_currents_alphas, stage_two_brackets_alphas
    ...
    def stage_two(self, a_low_init, a_high_init, phi, dphi_minus, dphi_plus,
        ...
        while iteration < max_iterations:
            ...
            brackets_xs = np.clip([x - a_low * starting_gradient, x - a_high * starting_gradient], -1,1)
            stage_two_brackets.append(brackets_xs)
            stage_two_brackets_alphas.append((a_low, a_high))
            iteration += 1
